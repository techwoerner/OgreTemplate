...
*/
if (!class_exists('acf_field_nav_menu_v5')) {
	...
	class acf_field_nav_menu_v5 extends acf_field
	{
		...
		*/
		function render_field( $field )
		{
			...
			// create Field HTML
			printf('<select id="%d" class="%s" name="%s">', $field['id'], $field['class'], $field['name']);
			...
			foreach( $nav_menus as $nav_menu_id => $nav_menu_name ) {
				...
				printf('<option value="%1$d" %3$s>%2$s</option>', $nav_menu_id, $nav_menu_name, $selected);
			}
